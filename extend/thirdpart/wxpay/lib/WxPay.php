...
//		}
//		$inputObj->SetAppid(config('wechat.wx_appid'));//公众账号ID
//		$inputObj->SetMch_id(config('wechat.mchid'));//商户号
//		$inputObj->SetNonce_str(self::getNonceStr());//随机字符串
        $inputObj['appid'] = config('wechat.wx_appid');
        $inputObj['mch_id'] = config('wechat.mchid');
        $inputObj['nonce_str'] = self::getNonceStr();

        $enterprise = new \extend\service\payment\WeChatEnterprisePayService();
        $s = $enterprise->getSign($inputObj);
        $inputObj['sign'] = $s;
		//$inputObj->SetSign()
		$xml = $enterprise->arrayToXml($inputObj);
		$startTimeStamp = self::getMillisecond();//请求开始时间
		//$response = self::postXmlCurl($xml, $url, true, $timeOut);
        $response = \extend\helper\Curl::postXmlSSLCurl($xml,$url,config('wechat.sslcert_path'), config('wechat.sslkey_path'));
		//$result = WxPayResults::Init($response);
        $result = \extend\helper\Utils::parseMsgData($response);
		self::reportCostTime($url, $startTimeStamp, $result);//上报请求花费时间
	 ...
	 * 
	 * 转换短链接
	 * 该接口主要用于扫码原生支付模式一中的二维码链接转成短链接(weixin://wxpay3/s/XXXXXX)，
	 * 减小二维码数据量，提升扫描速度和精确度。
	 * appid、mchid、spbill_create_ip、nonce_str不需要填入
